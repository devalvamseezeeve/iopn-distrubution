...
Some messages in iopn require permissions. For example, changing the mapping to support new CRC20 auto-conversion contracts or disabling the bridge module in case of emergency. Right now, only the admin has the ability to use those messages.
 The admin is a single address defined in iopn space and can be modified through governance. It is usually a multi-sig address shared by multiple trusted parties in order to achieve a higher level of security.
...
A special address defined in iopn space that can be modified through governance. Usually a multi-sig address.
...
### Option 1 : Create a permission system within iopn module for messages.
...
- Simple implementation self-contained in x/iopn module. Ability to be customized in the future if required.
...
### Option 2 : Use a subset of x/group module functionality to restrict some transactions in iopn

For the above problem, we propose the following:

- For each specific msg, we define a group and for the group, a group policy
- Use the admin defined in iopn space as general admin for the group
...
- completely remove (rollback) and be replaced by the circuit breaker
  (PR to rollback: https://github.com/devalvamseezeeve/iopn-distrubution/pull/795 and https://github.com/crypto-org-chain/gravity-bridge/pull/96)
...
- https://github.com/devalvamseezeeve/iopn-distrubution/pull/795
