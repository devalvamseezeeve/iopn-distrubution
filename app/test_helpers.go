...
func setup(withGenesis, skipGravity bool, invCheckPeriod uint) (*App, GenesisState) {
	...
	app := New(log.NewNopLogger(), db, nil, true, skipGravity, map[int64]bool{}, DefaultNodeHome, invCheckPeriod, encCdc, appOption, baseapp.SetChainID(TestAppChainID))
	if withGenesis {
...
func Setup(t *testing.T, iopnAdmin string, skipGravity bool) *App {
	t.Helper()
	...
	return SetupWithGenesisValSet(t, iopnAdmin, skipGravity, valSet, []authtypes.GenesisAccount{acc}, balance)
}
...
func SetupWithGenesisValSet(t *testing.T, iopnAdmin string, skipGravity bool, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *App {
	t.Helper()

	app, genesisState := setup(true, skipGravity, 5)
	genesisState = genesisStateWithValSet(t, app, genesisState, valSet, genAccs, balances...)

	iopnGen := iopntypes.DefaultGenesis()
	iopnGen.Params.CronosAdmin = iopnAdmin
	// enable auto deployment in test genesis
	iopnGen.Params.EnableAutoDeployment = true
	genesisState["iopn"] = app.cdc.MustMarshalJSON(iopnGen)
