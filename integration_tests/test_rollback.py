...
def update_node2_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:iopn_777-1-node{i}":
            ini[section].update(
                {
...
def custom_iopn(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-iopnd.nix",
    ]
    print(*cmd)
    broken_binary = Path(subprocess.check_output(cmd).strip().decode()) / "bin/iopnd"
    print(broken_binary)
...
def test_rollback(custom_iopn):
    ...
    print("stop node2")
    supervisorctl(custom_iopn.base_dir / "../tasks.ini", "stop", "iopn_777-1-node2")

    print("do rollback on node2")
    cli2.rollback(keyring_backend="test")

    print("switch to normal binary")
    update_node2_cmd(custom_iopn.base_dir, "iopnd", 2)
    supervisorctl(custom_iopn.base_dir / "../tasks.ini", "update")
    wait_for_port(ports.rpc_port(custom_iopn.base_port(2)))
