...
def test_evm_update_param(iopn, tmp_path):
    ...
    with pytest.raises(ValueError) as e_info:
        deploy_contract(iopn.w3, CONTRACTS["Greeter"])
    assert invalid_msg in str(e_info.value)


def test_gov_update_params(iopn, tmp_path):
    cli = iopn.cosmos_cli()
    ...
    params = {
        "iopn_admin": "crc12luku6uxehhak02py4rcz65zu0swh7wjsrw0pp",
